...
if [[ "$1" != 'bash' && "$1" != 'sh' && "$1" != '/bin/sh' ]]; then
    ...
    echo "init     | upgrade_check_result:" "$upgrade_check_result"
    ...
    provisioning_status=$(provision_check_status ${provisioning_cache})
    echo 'init     | Provisioning status:' ${provisioning_status}
    if [[ ${provisioning_status} != "PROVISIONED" ]]; then
        echo 'init     | Scheduled provisioning..'
        perform_provision ${IGNITION_INSTALL_LOCATION} "${GATEWAY_PROVISION_TYPE:=DEV}" &
    else
        # install latest modules
        register-modules.sh true "${IGNITION_INSTALL_LOCATION}/data/db/config.idb"
    fi

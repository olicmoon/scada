...
function main() {
    if [ ! -d "/modules" ]; then
        ...
        echo "init     | WARNING: ${DB_LOCATION} not found, skipping module registration"
        return 0
    fi
...
function register_modules() {
    ...
    for module in /modules/*.modl; do
        ...
        cp "${module_sourcepath}" "${module_destpath}"
        chown ignition:ignition "${module_destpath}"
        echo "init     | chmod: ${module_basename}"
